...
如果分组列中包含具有NULL值的行,则NULL将作为一个分组返回,如果列中含有多行NULL值,它们将分为一组GROUP BY 子句必须出现在WHERE子句之后,ORDER BY子句之前
...
学习目标//:另外一些联结(含义和用法),介绍如何使用表别名,如何对被联结的表使用聚集函数
...
☆用自联结而不用子查询:因为自联结查询速度比子查询快的多,性能更好



13.2.2自然联结:排除多次出现,只返回一次 .  联结的 另一个目的在于产生对应关系
...
13.2.3外联结  outer join

许多联结将一个表中的行与另一个表中的行想关联,但有时需要包含没有关联行的那些行.
