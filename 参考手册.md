...
```json
{
    ...
    "emmet.triggerExpansionOnTab": true,  //按tab键补全
    // 文件类型 : 扩展html支持
	// 使得vue文件可以支持ul>li等写法
    "emmet.includeLanguages": {
        "vue-html": "html", 
...
[ppa安装](https://github.com/nodesource/distributions#deb)
...
[官方命令详解](https://docs.npmjs.com/cli/v7/commands)
...
> - 创建虚拟环境
> - 进入, 退出虚拟环境
> - 为pycharm配置虚拟环境(其实就是选择解释器)

## 虚拟环境的作用

创建独立的python环境, 环境间相互独立, 互不影响

1. 在没有权限的情况下安装新套件
2. 不同应用可以使用不同的套件版本
3. 套件升级不影响其他应用

应用场景:不同项目和应用间  或者用于生产和测试和部署验证

不同项目使用不同的版本系统彼此之间相互隔离, 避免冲突(相当于沙盒,相互独立)

作用: 搭建独立的python运行环境, 使得

创建完成虚拟环境后会自动进入

## 虚拟环境搭建

[参考文档](https://docs.python.org/zh-cn/3/tutorial/venv.html)
...
```bash
# 创建 tutorial-env 目录名
python3 -m venv --without-pip tutorial-env
# 激活创建的虚拟环境
...
Dependency Installed:keyutils-libs-devel.x86_64 0:1.5.8-3.el7     krb5-devel.x86_64 0:1.15.1-50.el7       libcom_err-devel.x86_64 0:1.42.9-19.el7     libselinux-devel.x86_64 0:2.5-15.el7libsepol-devel.x86_64 0:2.5-10.el7           libverto-devel.x86_64 0:0.2.5-4.el7     pcre-devel.x86_64 0:8.32-17.el7
```
...
[参考链接](https://pip.pypa.io/en/stable/installing/)
...
```bash
sudo apt install python3-testresources python3-venv
```
...
# 配置文件路径 需要自己创建
~/.pip/pip.conf
# 复制下方内容到 文件中 意思是全局阿里镜像源 并且是被信任的主机
```
...
| 笔记链接            | Alt + Shift + L   |                                 |
| 修改标题 / 重命名   | F2                |                                 |
| 搜索笔记            | Ctrl + Q          | 搜索笔记 标签 Esc退出或光标退出 |
| 查找文本            | Ctrl + F          |                                 |
| 全局查找 / 调起笔记 | Win + Shift + F   |                                 |
|                     |                   |                                 |
|                     |                   |                                 |
|                     |                   |                                 |
|                     |                   |                                 |
...
保存方式： 文章  微博 分享 微信
Ctrl +B :先加粗 输入指定的文本后,
使用：ctrl+Q:快速查找 Tag 标签查找分类
合并功能
ctrl+shift+v粘贴为纯文本
超链接  页内超链接 本地超链接  工具i
F6：快速搜索
Ctrl+Alt+L：链接笔记复制笔记链接
CTrl+shift+H：高亮
F10：展开折叠笔记本
F11：打开关闭笔记列表
ctrl+F11 ：打开关闭笔记编辑界面
缩进 ctrl+shift+M：向前缩进 | ctrl+M：向后缩进
Ctrl+;:插入当前时间
先使用Markdown编辑,再用默认格式
...
| 需求/问题        | 解决方案                                |
| ---------------- | --------------------------------------- |
| Markdown转富文本 | 预览模式下选中全部,粘贴保留格式         |
| 选择哪种笔记方式 | 优先Markdown,因为Markdown可以转为富文本 |
|                  |                                         |
...
特性: 自动补全 	可选参数	提示(ctrl + P)	图片插件

# Docker - 容器

[本地安装](D:\Recv Files\docker)
...
```bash
# 新版文档搜索
site：gitbook.io redis
# 旧版文档搜索
site: gitbook.com redis
```
...
```
五角星 wjx=☆ 圆圈 yq=○ 菱形 lx=◇ 方块 fk=□ 三角形 sjx=△ 
五角星 wjx=★ 圆圈 yq=● 菱形 lx=◆ 方块 fk=■ 三角形 sjx=▲ 
空格 kg=　 逗号 dh=， 句号 jh=。 圆点 yd=· 破折号 pzh=—— 
双空格 skg=　　 顿号 dh=、 波浪号 blh=～ 间隔号 jgh=· 省略号 slh=…… 
单引号 dyh=‘’ 方括号 fkh=〔〕 书名号 smh=《》 大括号 dkh=｛｝ 对比号 dbh=∶ 
双引号 syh=“” 正负号 zfh=± 单书名号 dsmh=〈〉 圆括号 ykh=（） 斜杠 xg=／ 
美元 my=＄ 竖书名号 ssmh=『』 空心括号 kxkh=〖〗 性别 xb=♂ 雄性符号 xxfh=♂ 
英镑 yb=￡ 对勾 dg=√ 实心括号 sxkh=【】 性别 xb=♀ 雌性符号 cxfh=♀ 
人民币 rmb=￥ 双竖线 ssx=‖ 单撇号 dph=′ 双撇号 sph=″ 摄氏度 ssd=℃ 
No no=№ 度 du=° 分 fen=′ 秒 miao=″ 温度 wb=℃ 
Num num=№ 章节号 zjh=§ 花叉号 hch=※ 左箭头 zjt=← 上箭头 sjt=↑ 
序号 num=№ 千分号 qfh=‰ 花a ha=＠ 右箭头 yjt=→ 下箭头 xjt=↓ 
问号 wh=？ 井号 jh=＃ and and=＆ 加号 jiah=＋ 乘号 ch=× 
叹号 th=！ 百分号 bfh=％ 和 he=＆ 减号 jianh=－ 除号 ch=÷ 
冒号 mh=： 小于 xy=＜ 等于 dy=＝ 小于等于 xydy=≤ PAI pai=π 
分号 fh=； 大于 dy=＞ 约等于 ydy=≈ 大于等于 dydy=≥ － －
```
...
[参考文档](https://wsgzao.github.io/post/lrzsz/)
...
最好使用 http://www.google.com/ncr `NCR: No Country Redirection`，而不是`http://www.google.com.hk`；有时，直接输入`http://www.google.com`也会被自动转到“本地Google”，比如，我用日本的 VPN，浏览器就会把我转到`http://www.google.co.jp`……
...
再学一个在指定网站中搜索的语法“site:”。比如，`“the purpose of education” site:http://www.time.com/`就是要求 Google 只返回 [http://www.time.com](http://www.time.com/) 这个网站里的含有“the purpose of education”的文章。
